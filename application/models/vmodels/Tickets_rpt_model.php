...
class Tickets_rpt_model extends CI_Model {
    ...
    public function queryComplete($data, $sorder_by="a.fst_ticket_no", $rptLayout="1") {
        ...
        $userActive = $this->aauth->user();
        $userActiveLevel = $userActive->fin_level;
        ...
        if ($rptLayout == "1"){
            $swhere = " and (
                    (b.fin_department_id = $department_id  and g.fin_level > $userActiveLevel) 
                    or (e.fin_department_id = $department_id and h.fin_level > $userActiveLevel)
                    or (f.fin_department_id = $department_id and i.fin_level > $userActiveLevel)
                    or b.fin_user_id = $userActive->fin_user_id
                    or e.fin_user_id = $userActive->fin_user_id
                    or f.fin_user_id = $userActive->fin_user_id
                )";
            ...
            //if ($department_id > "0") {
                ...
                //$swhere .= " and (b.fin_department_id = " . $this->db->escape($department_id) ." or e.fin_department_id = " . $this->db->escape($department_id) .")";
            //}
            ...
            if ($user_id > "0" ) {
                ...
                $swhere .= " and (b.fin_user_id = " . $this->db->escape($user_id) ." or e.fin_user_id = " . $this->db->escape($user_id) ." or f.fin_user_id = " . $this->db->escape($user_id) .")";
            }
            if ($ticket_type > "0" ) {
        ...
        if ($rptLayout == "2"){
            $swhere = " and (
                (a.fin_department_id = $department_id  and e.fin_level > $userActiveLevel) 
                or a.fin_user_id = $userActive->fin_user_id
            )";
            if ($branch_id > "0") {
        ...
        if ($rptLayout == "3"){
            $swhere = " and (
                (d.fin_department_id = $department_id  and h.fin_level > $userActiveLevel) 
                or (e.fin_department_id = $department_id and i.fin_level > $userActiveLevel)
                or (g.fin_department_id = $department_id and j.fin_level > $userActiveLevel)
                or d.fin_user_id = $userActive->fin_user_id
                or e.fin_user_id = $userActive->fin_user_id
                or g.fin_user_id = $userActive->fin_user_id
            )";
            if ($branch_id > "0") {
            ...
            if ($department_id > "0") {
                ...
                $swhere .= " and (d.fin_department_id = " . $this->db->escape($department_id) ." or e.fin_department_id = " . $this->db->escape($department_id) .")";
            }
            if ($userGroup_id > "0") {
                $swhere .= " and d.fin_group_id = " . $this->db->escape($userGroup_id);
            }
            if ($user_id > "0" ) {
                $swhere .= " and (d.fin_user_id = " . $this->db->escape($user_id) ." or e.fin_user_id = " . $this->db->escape($user_id) ." or g.fin_user_id = " . $this->db->escape($user_id) .")";
            }
            if ($ticket_type > "0" ) {
                $swhere .= " and a.fin_ticket_type_id = " . $this->db->escape($ticket_type);
            }
        ...
        switch($rptLayout) {
            case "1":
                ...
                LEFT JOIN users f on a.fin_approved_by_user_id = f.fin_user_id 
                LEFT JOIN usersgroup g ON g.fin_group_id = b.fin_group_id
                LEFT JOIN usersgroup h ON h.fin_group_id = e.fin_group_id
                LEFT JOIN usersgroup i ON i.fin_group_id = f.fin_group_id
                " . $swhere . $sorderby;
                break;
            case "2":
                $swhere2 = "";
            ...
            case "3":
                ...
                LEFT JOIN usersgroup h ON h.fin_group_id = d.fin_group_id
                LEFT JOIN usersgroup i ON i.fin_group_id = e.fin_group_id
                LEFT JOIN usersgroup j ON j.fin_group_id = g.fin_group_id " . $swhere . $sorderby;
                break;
            default:
                break;
        }
    ...
    public function queryCompleteAdmin($data, $sorder_by="a.fst_ticket_no", $rptLayout="1") {
        ...
        if ($rptLayout == "1"){
            ...
            if ($user_id > "0" ) {
                //$swhere .= " and b.fin_user_id = " . $this->db->escape($user_id);
                $swhere .= " and (b.fin_user_id = " . $this->db->escape($user_id) ." or e.fin_user_id = " . $this->db->escape($user_id) .")";
            }
            if ($ticket_type > "0" ) {
                $swhere .= " and a.fin_ticket_type_id = " . $this->db->escape($ticket_type);
            }
        ...
        if ($rptLayout == "3"){
            ...
            if ($user_id > "0" ) {
                $swhere .= " and (d.fin_user_id = " . $this->db->escape($user_id) ." or e.fin_user_id = " . $this->db->escape($user_id) .")";
            }
            if ($ticket_type > "0" ) {
                $swhere .= " and a.fin_ticket_type_id = " . $this->db->escape($ticket_type);
            }
