...
class Ticketstatus_model  extends MY_MODEL {
    ...
    public function get_our_tickets_notif(){
        $user = $this->aauth->user();
        $userActive = $user->fin_user_id;
        $deptActive = $user->fin_department_id;
        $levActive = intval($user->fin_level);
        ...
        OR (d.fin_department_id = $deptActive AND d.fin_level > $levActive)
        OR (e.fin_department_id = $deptActive AND e.fin_level > $levActive))
        AND a.fst_status !='REJECTED' AND a.fst_status !='CLOSED' AND a.fst_status !='VOID' AND a.fst_status !='ACCEPTANCE_EXPIRED' AND a.fst_status !='APPROVAL_EXPIRED' AND a.fst_status !='TICKET_EXPIRED'
        AND NOT FIND_IN_SET (". $userActive.",a.fst_view_id) ";
        $qr = $this->db->query($ssql,[]);
        //echo $this->db->last_query();
        //die();
        return $qr->result_array();
