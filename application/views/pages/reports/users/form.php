    ...
    <div class="box-body">
        <input type="hidden" name="<?= $this->security->get_csrf_token_name() ?>" value="<?= $this->security->get_csrf_hash() ?>">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-4">								
                    <label class="radio"><input type="radio" id="rpt_layout1" class="rpt_layout" name="rpt_layout" value="1" checked onclick="handleRadioClick(this);"><?=lang("Users Report")?></label>
                </div>
...
<script type="text/javascript">
    ...
    $(function() {
		...
		$("#select-department").select2({
			width: '100%',
			ajax: {
				url: '<?=site_url()?>report/users/get_department',
				dataType: 'json',
				delay: 250,
				processResults: function (data){
					items = [];
		...
		$("#select-groupId").select2({
			...
			tokenSeparators: [",", " "],
			ajax: {
				url: '<?=site_url()?>report/users/get_usergroup',
				dataType: 'json',
				delay: 250,
				processResults: function (data){
					items = [];
