    ...
    <div class="row">
        <div class="col-xs-12 col-md-12">
            <div class="box box-info">
                ...
                <!-- form start -->
                <form id="frmTicketType" class="form-horizontal" action="<?= site_url() ?>master/tickettype/add" method="POST" enctype="multipart/form-data">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-xs-6 col-md-4">
                                <select class="form-control" id="fst_assignment_or_notice" name="fst_assignment_or_notice">
                                    ...
                                    <option value='ASSIGNMENT'><?=lang("ASSIGNMENT")?></option>
                                    <option value='NOTICE'><?=lang("NOTICE")?></option>
...
<script type="text/javascript">
    $(function(){
        <?php if($mode == "EDIT"){?>
            init_form($("#fin_ticket_type_id").val());
        <?php } ?>

        $("#btnSubmitAjax").click(function(event){
            event.preventDefault();
            //data = new FormData($("#frmTicketType")[0]);
            data = $("#frmTicketType").serializeArray();

            mode = $("#frm-mode").val();
            if (mode == "ADD"){
                url = "<?= site_url() ?>master/tickettype/ajx_add_save";
            }else{
                url = "<?= site_url() ?>master/tickettype/ajx_edit_save";
            }
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    if (resp.status == "VALIDATION_FORM_FAILED"){
                        ...
                        data = resp.data;
                        $("#fin_ticket_type_id").val(data.insert_id);
                        ...
                        $('#fst_ticket_type_name').prop('readonly', true);
                    }
                },
                error: function (e) {
        ...
        $("#btnNew").click(function(e){
            e.preventDefault();
            window.location.replace("<?=site_url()?>master/tickettype/add")
        });
        ...
        $("#btnDelete").click(function(e){
            ...
            $.ajax({
                url:"<?= site_url() ?>master/tickettype/delete/" + $("#fin_ticket_type_id").val(),
            }).done(function(resp){
                //consoleLog(resp):
                $.unblockUI();
                if (resp.message != "") {
                    $.alert({
                        ...
                        buttons: {
                            OK : function() {
                                if (resp.status == "SUCCESS") {
                                    window.location.href = "<?=site_url() ?>master/tickettype/lizt";
                                    return;
                                }
                            },
                        }
        ...
        $("#btnList").click(function(e){
            e.preventDefault();
            window.location.replace("<?=site_url()?>master/tickettype/lizt");
        });
    })

    function init_form(fin_ticket_type_id){
        //alert("Init Form);
        var url = "<?=site_url()?>master/tickettype/fetch_data/" + fin_ticket_type_id;
        $.ajax({
            type: "GET",
            url: url,
            success: function (resp) {
                console.log(resp.ticketType);

                $.each(resp.ticketType, function(name, val){
                    var $el = $('[name="'+name+'"]'),
                    type = $el.attr('type');
                    switch(type){
                        case 'checkbox':
